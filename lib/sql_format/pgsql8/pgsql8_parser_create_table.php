 ...
 * @version $Id: pgsql8_parser_create_table.php 2261 2012-01-09 08:37:44Z nkiraly $
 */
    ...
    while (strlen($line) > 0) {
      ...
      for($i = 2; $i < count($tokens); $i++) {
        if ( strcasecmp($tokens[$i], 'UNIQUE') == 0 ) {
          ...
          //dbsteward::console_line(3, "NOTICE:  CREATE TABLE with UNIQUE column attribute -- creating implicit index \"" . pgsql8::index_name(sql_parser::get_object_name($node_table->get_name()), sql_parser::get_object_name($tokens[0]), 'key') . "\" for table \"" . $node_schema->get_name() . '.' . $node_table->get_name() . "\"");
