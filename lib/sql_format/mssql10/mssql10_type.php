 ...
 * $Id: mssql10_type.php 2261 2012-01-09 08:37:44Z nkiraly $
 */
...
class mssql10_type {
  ...
  public static function get_add_check_sql($node_schema, $node_table, $column, $node_type) {
    ...
    $column_name = mssql10_diff::get_quoted_name($column['name'], dbsteward::$quote_column_names);
  ...
  public static function get_creation_sql($node_schema, $node_type) {
    ...
    $reference_table_name = mssql10_diff::get_quoted_name($node_schema['name'], dbsteward::$quote_schema_names) . '.' . mssql10_diff::get_quoted_name($node_type['name'] . '_enum_values', dbsteward::$quote_table_names);
  ...
  public static function column_constraint_temporary_drop(&$columns, $node_schema, $node_type) {
    ...
    for($i=0; $i < count(mssql10_diff::$new_table_dependency); $i++) {
      ...
      foreach(dbx::get_table_columns($new_table) AS $new_column) {
        ...
        $unquoted_type_name = mssql10_diff::get_quoted_name($node_schema['name'], false) . '.' . mssql10_diff::get_quoted_name($node_type['name'], false);
