 ...
 * @version $Id: mssql10_diff_tables.php 2261 2012-01-09 08:37:44Z nkiraly $
 */
        ...
        if ($db_doc_new_schema) {
          ...
          if ($db_doc_new_table) {
            ...
            if ($db_doc_new_column) {
              if (isset($db_doc_new_column['afterAddPreStage1'])) {
                $commands[] = array('stage' => '1preentire',
                  'command' => trim($db_doc_new_column['afterAddPreStage1']) . " -- from " . $new_schema['name'] . "." . $new_table['name'] . "." . $new_column['name'] . " afterAddPreStage1 definition");
              }
              if (isset($db_doc_new_column['afterAddPostStage1'])) {
                $commands[] = array('stage' => '1postentire',
    ...
    if ($old_schema != NULL) {
      foreach (dbx::get_tables($old_schema) as $table) {
        ...
        if (!mssql10_schema::contains_table($new_schema, $table['name'])) {
          if ( !dbsteward::$ignore_oldname && ($renamed_table_name = mssql10_schema::table_name_by_old_name($new_schema, $table['name'])) !== false ) {
            // table indicating oldName = table['name'] present in new schema? don't do DROP statement
        ...
        if ($old_table_rows && $new_table_rows) {
          ...
          for ($i = 0; $i < $count_old_rows; $i++) {
            ...
            if (count($changes[$i]) > 0) {
              ...
              $sql .= mssql10_diff_tables::get_data_row_update($new_schema, $new_table, $primary_key_index, $old_table_row_columns, $old_rows[$i], $new_table_row_columns, $new_rows[$i], $changes[$i]);
            }
          }
        }
