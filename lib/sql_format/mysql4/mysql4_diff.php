 ...
 * @version $Id: mysql4_diff.php 2274 2012-01-10 19:33:52Z nkiraly $
 */

require_once dirname(__FILE__) . '/mysql4_diff_indexes.php';
require_once dirname(__FILE__) . '/mysql4_diff_functions.php';
require_once dirname(__FILE__) . '/mysql4_diff_sequences.php';
require_once dirname(__FILE__) . '/mysql4_diff_tables.php';
require_once dirname(__FILE__) . '/mysql4_diff_triggers.php';
require_once dirname(__FILE__) . '/mysql4_diff_types.php';
require_once dirname(__FILE__) . '/mysql4_diff_views.php';
  ...
  private static function update_structure($fp1, $fp2) {
    ...
    if (!is_array(mysql4_diff::$new_table_dependency)) {
      ...
      for ($i = 0; $i < count(mysql4_diff::$new_table_dependency); $i++) {
        ...
        $item = mysql4_diff::$new_table_dependency[$i];
      ...
      for ($i = count(mysql4_diff::$old_table_dependency) - 1; $i >= 0; $i--) {
        // find the necessary pointers
        $item = mysql4_diff::$old_table_dependency[$i];
  ...
  protected static function update_permissions($fp1, $fp2) {
    foreach (dbx::get_schemas(dbsteward::$new_database) AS $new_schema) {
      ...
      foreach (dbx::get_tables($new_schema) AS $new_table) {
        ...
        if ( !dbsteward::$ignore_oldname && mysql4_diff_tables::is_renamed_table($old_schema, $new_schema, $new_table) ) {
      ...
      foreach (dbx::get_functions($new_schema) AS $new_function) {
        $old_function = NULL;
        if ($old_schema != NULL) {
          $old_function = dbx::get_function($old_schema, $new_function['name'], mysql4_function::get_declaration($new_schema, $new_function));
        }
      ...
      foreach (dbx::get_views($new_schema) AS $new_view) {
        ...
        foreach (dbx::get_permissions($new_view) AS $new_permission) {
          ...
          || !mysql4_permission::has_permission($old_view, $new_permission)
  ...
  private static function update_data($fp, $delete_mode = FALSE) {
    if (mysql4_diff::$new_table_dependency != NULL && count(mysql4_diff::$new_table_dependency) > 0) {
      for ($i = 0; $i < count(mysql4_diff::$new_table_dependency); $i++) {
        ...
        if ($delete_mode) {
          $item = mysql4_diff::$new_table_dependency[count(mysql4_diff::$new_table_dependency) - 1 - $i];
        }
        else {
          $item = mysql4_diff::$new_table_dependency[$i];
        }
