...
class mssql10_function extends pgsql8_function {
  ...
  public function get_drop_sql($node_schema, $node_function) {
    ...
    $ddl = "DROP FUNCTION " . mssql10::get_quoted_name($node_schema['name'], dbsteward::$quote_schema_names) . '.' . mssql10::get_quoted_name($node_function['name'], dbsteward::$quote_function_names) . ';';

    // MSSQL procedure mode?
    if (isset($node_function['procedure'])) {
      $ddl = "DROP PROCEDURE " . mssql10::get_quoted_name($node_schema['name'], dbsteward::$quote_schema_names) . '.' . mssql10::get_quoted_name($node_function['name'], dbsteward::$quote_function_names) . ';';
    }
  ...
  public static function get_declaration($node_schema, $node_function) {
    $r = mssql10::get_quoted_name($node_schema['name'], dbsteward::$quote_schema_names) . '.' . mssql10::get_quoted_name($node_function['name'], dbsteward::$quote_function_names) . '(';
