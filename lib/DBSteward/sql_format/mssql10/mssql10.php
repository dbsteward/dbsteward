...
class mssql10 {
  ...
  public function build_upgrade($old_files, $new_files) {
    ...
    $upgrade_files = mssql10_diff::diff_doc($old_composite_file, $new_composite_file, $old_db_doc, $new_db_doc, $upgrade_prefix);
  ...
  public static function value_escape($type, $value, $db_doc = NULL) {
    if (strlen($value) > 0) {
      ...
      // time with time zone is converted to time in xml_parser::mssql10_type_convert()
