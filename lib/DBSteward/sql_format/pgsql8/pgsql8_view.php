...
class pgsql8_view {
  ...
  public function get_creation_sql($node_schema, $node_view) {
    ...
    $view_name = pgsql8::get_quoted_name($node_schema['name'], dbsteward::$quote_schema_names) . '.' . pgsql8::get_quoted_name($node_view['name'], dbsteward::$quote_table_names);
  ...
  public function get_drop_sql($node_schema, $node_view) {
    $ddl = "DROP VIEW " . pgsql8::get_quoted_name($node_schema['name'], dbsteward::$quote_schema_names) . '.' . pgsql8::get_quoted_name($node_view['name'], dbsteward::$quote_table_names) . ";\n";
    return $ddl;
  }
