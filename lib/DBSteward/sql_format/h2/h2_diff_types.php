...
class h2_diff_types {
  ...
  public static function apply_changes($ofs, $old_schema, $new_schema) {
    ...
    foreach (dbx::get_types($new_schema) AS $new_type) {
      ...
      if (($old_schema == NULL) || !h2_schema::contains_type($old_schema, $new_type['name'])) {
        continue;
      }
      ...
      if ( h2_type::equals($old_schema, $old_type, $new_schema, $new_type) ) {
        continue;
      }
    }
  }
    ...
    if ($old_schema != NULL) {
      foreach(dbx::get_types($old_schema) AS $type) {
        if ( !format_schema::contains_type($new_schema, $type['name'])) {
          ...
          // $ofs->write(h2_type::get_type_demotion_sql($new_schema, $type));
        }
      }
    }
  }
