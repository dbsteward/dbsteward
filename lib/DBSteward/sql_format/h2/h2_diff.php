...
class h2_diff extends sql99_diff {
  ...
  public static function revoke_permissions($ofs1, $ofs3) {
    foreach (dbx::get_schemas(dbsteward::$old_database) as $old_schema) {
      ...
      foreach ( $old_permissions as $node_permission ) {
        ...
        $ofs1->write(h2_permission::get_permission_sql(dbsteward::$old_database, $old_schema, $node_object, $node_permission, 'revoke')."\n");
      }
    }
  }
  ...
  public static function update_permissions($ofs1, $ofs3) {
    foreach (dbx::get_schemas(dbsteward::$new_database) as $new_schema) {
      ...
      foreach ( $permissions as $node_permission ) {
        ...
        $ofs1->write(h2_permission::get_permission_sql(dbsteward::$new_database, $new_schema, $node_object, $node_permission, 'grant')."\n");
      }
    }
  }
  ...
  protected static function drop_old_schemas($ofs) {
    ...
    if (is_array(h2_diff::$old_table_dependency)) {
      $deps = h2_diff::$old_table_dependency;
        ...
        if (!dbx::get_schema(dbsteward::$new_database, $old_schema['name'])) {
          ...
          if (!in_array(trim($old_schema['name']), $processed_schemas)) {
            ...
            foreach ($old_schema->type as $node_type) {
              $ofs->write(h2_type::get_drop_sql($old_schema, $node_type) . "\n");
            }
            foreach ($old_schema->function as $node_function) {
              $ofs->write(h2_function::get_drop_sql($old_schema, $node_function) . "\n");
            }
            foreach ($old_schema->sequence as $node_sequence) {
              $ofs->write(h2_sequence::get_drop_sql($old_schema, $node_sequence) . "\n");
            }
          ...
          // $ofs->write(h2_table::get_drop_sql($old_schema, $dep['table']) . "\n");
          $table_name = h2::get_fully_qualified_table_name($dep['schema']['name'], $dep['table']['name']);
          ...
          foreach (h2_table::get_sequences_needed($old_schema, $dep['table']) as $node_sequence) {
            $ofs->write(h2_sequence::get_drop_sql($old_schema, $node_sequence) . "\n");
          }
        }
      }
      ...
      foreach (dbsteward::$old_database->schema as $old_schema) {
        if (!dbx::get_schema(dbsteward::$new_database, $old_schema['name'])) {
          ...
          foreach ($old_schema->table as $node_table) {
            // tables will drop themselves later on
            // $ofs->write(h2_table::get_drop_sql($old_schema, $node_table) . "\n");
            $table_name = h2::get_fully_qualified_table_name($old_schema['name'], $node_table['name']);
            $ofs->write("-- $table_name triggers, indexes, constraints will be implicitly dropped when the table is dropped\n");
            $ofs->write("-- $table_name will be dropped later according to table dependency order\n");
            foreach (h2_table::get_sequences_needed($old_schema, $node_table) as $node_sequence) {
              $ofs->write(h2_sequence::get_drop_sql($old_schema, $node_sequence) . "\n");
            }
          }
        }
  ...
  public static function update_structure($ofs1, $ofs3) {
    if (!h2::$use_schema_name_prefix) {
      if (count(dbsteward::$new_database->schema) > 1) {
        throw new Exception("You cannot use more than one schema in h2 without schema name prefixing\nPass the --useschemaprefix flag to turn this on");
      }
    ...
    h2_diff_views::drop_views_ordered($ofs1, dbsteward::$old_database, dbsteward::$new_database);
    
    //@TODO: implement h2_language ? no relevant conversion exists see other TODO's stating this
    //h2_diff_languages::diff_languages($ofs1);
    // if the table dependency order is unknown, bang them in natural order
    if (!is_array(h2_diff::$new_table_dependency)) {
      foreach (dbx::get_schemas(dbsteward::$new_database) AS $new_schema) {
        ...
        h2_diff_types::apply_changes($ofs1, $old_schema, $new_schema);
        h2_diff_functions::diff_functions($ofs1, $ofs3, $old_schema, $new_schema);
        h2_diff_sequences::diff_sequences($ofs1, $ofs3, $old_schema, $new_schema);
        // remove old constraints before table contraints, so the SQL statements succeed
        h2_diff_constraints::diff_constraints($ofs1, $old_schema, $new_schema, 'constraint', TRUE);
        h2_diff_constraints::diff_constraints($ofs1, $old_schema, $new_schema, 'primaryKey', TRUE);
        h2_diff_tables::drop_tables($ofs3, $old_schema, $new_schema);
        h2_diff_tables::diff_tables($ofs1, $ofs3, $old_schema, $new_schema);
        // h2_diff_indexes::diff_indexes($ofs1, $old_schema, $new_schema);
        h2_diff_constraints::diff_constraints($ofs1, $old_schema, $new_schema, 'primaryKey', FALSE);
        h2_diff_triggers::diff_triggers($ofs1, $old_schema, $new_schema);
      }
      ...
      foreach (dbx::get_schemas(dbsteward::$new_database) AS $new_schema) {
        $old_schema = dbx::get_schema(dbsteward::$old_database, $new_schema['name']);
        h2_diff_constraints::diff_constraints($ofs1, $old_schema, $new_schema, 'constraint', FALSE);
      }
    }
    else {
      $processed_schemas = array();
      for ($i = 0; $i < count(h2_diff::$new_table_dependency); $i++) {
        // find the necessary pointers
        $item = h2_diff::$new_table_dependency[$i];
      ...
      for ($i = count(h2_diff::$old_table_dependency) - 1; $i >= 0; $i--) {
        // find the necessary pointers
        $item = h2_diff::$old_table_dependency[$i];
        ...
        // as h2_diff_tables::diff_constraints_table() will do rename checking when recreating constraints for renamed tables

        h2_diff_constraints::diff_constraints_table($ofs1, $old_schema, $old_table, $new_schema, $new_table, 'constraint', TRUE);
        h2_diff_constraints::diff_constraints_table($ofs1, $old_schema, $old_table, $new_schema, $new_table, 'primaryKey', TRUE);
      }
      ...
      for ($i = 0; $i < count(h2_diff::$new_table_dependency); $i++) {
        ...
        h2_diff_tables::diff_tables($ofs1, $ofs3, $old_schema, $new_schema, $old_table, $new_table);
        // h2_diff_indexes::diff_indexes_table($ofs1, $old_schema, $old_table, $new_schema, $new_table);
        h2_diff_constraints::diff_constraints_table($ofs1, $old_schema, $old_table, $new_schema, $new_table, 'primaryKey', FALSE);
        h2_diff_triggers::diff_triggers_table($ofs1, $old_schema, $old_table, $new_schema, $new_table);
        h2_diff_constraints::diff_constraints_table($ofs1, $old_schema, $old_table, $new_schema, $new_table, 'constraint', FALSE);
      }
      ...
      for ($i = count(h2_diff::$old_table_dependency) - 1; $i >= 0; $i--) {
        ...
        h2_diff_tables::drop_tables($ofs3, $old_schema, $new_schema, $old_table, $new_table);
      }
    }
    
    h2_diff_views::create_views_ordered($ofs3, dbsteward::$old_database, dbsteward::$new_database);
  }
  ...
  private static function update_data($ofs, $delete_mode = FALSE) {
    if (h2_diff::$new_table_dependency != NULL && count(h2_diff::$new_table_dependency) > 0) {
      for ($i = 0; $i < count(h2_diff::$new_table_dependency); $i++) {
        // go in reverse when in delete mode
        if ($delete_mode) {
          $item = h2_diff::$new_table_dependency[count(h2_diff::$new_table_dependency) - 1 - $i];
        }
        else {
          $item = h2_diff::$new_table_dependency[$i];
        }
        ...
        $ofs->write(h2_diff_tables::get_data_sql($old_schema, $old_table, $new_schema, $new_table, $delete_mode));
      }
    }
    else {
      ...
      foreach (dbx::get_schemas(dbsteward::$new_database) AS $new_schema) {
        $old_schema = dbx::get_schema(dbsteward::$old_database, $new_schema['name']);
        h2_diff_tables::diff_data($ofs, $old_schema, $new_schema);
      }
    }
  }
