...
class h2_diff_sequences extends sql99_diff_sequences {
  ...
  public static function diff_sequences($ofs1, $ofs3, $old_schema, $new_schema) {
    ...
    if ( empty($new_sequences) ) {
      ...
      if ( !empty($old_sequences) ) {
        $ofs3->write(h2_sequence::get_shim_drop_sql());
      }
    }
    else {
      ...
      if ( empty($old_sequences) ) {
        $ofs1->write(h2_sequence::get_shim_creation_sql()."\n\n");
        $ofs1->write(h2_sequence::get_creation_sql($new_schema, $new_sequences)."\n");
      }
      else {
        ...
        if ( ! empty($to_drop) ) {
          $ofs1->write(h2_sequence::get_drop_sql($old_schema, $to_drop)."\n\n");
        }
        ...
        if ( ! empty($to_insert) ) {
          $ofs1->write(h2_sequence::get_creation_sql($new_schema, $to_insert)."\n");
        }
      }
    }
  }
  ...
  private static function schema_contains_sequence($schema, $sequence_name, $include_oldnames=FALSE) {
    if ( h2_schema::contains_sequence($schema, $sequence_name) ) {
      return TRUE;
    }

    foreach ( dbx::get_tables($schema) as $table ) {
      foreach ( h2_table::get_sequences_needed($schema, $table) as $sequence ) {
  ...
  private static function diff_single($ofs, $old_seq, $new_seq) {
    ...
    if ( !dbsteward::$ignore_oldnames && !empty($new_seq['oldSequenceName']) ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::SEQ_COL) . " = '{$new_seq['name']}'";
    }

    if ( $new_seq['inc'] == null && $old_seq['inc'] != null ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::INC_COL) . ' = DEFAULT';
    }
    if ( $new_seq['inc'] != null && strcasecmp($new_seq['inc'], $old_seq['inc']) != 0 ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::INC_COL) . ' = ' . $new_seq['inc'];
    }

    if ( $new_seq['min'] == null && $old_seq['min'] != null ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::MIN_COL) . ' = DEFAULT';
    }
    elseif ( $new_seq['min'] != null && strcasecmp($new_seq['min'], $old_seq['min']) != 0 ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::MIN_COL) . ' = ' . $new_seq['min'];
    }

    if ( $new_seq['max'] == null && $old_seq['max'] != null ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::MAX_COL) . ' = DEFAULT';
    }
    elseif ( $new_seq['max'] != null && strcasecmp($new_seq['max'], $old_seq['max']) != 0 ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::MAX_COL) . ' = ' . $new_seq['max'];
    }

    if ( $new_seq['start'] != null && strcasecmp($new_seq['start'], $old_seq['start']) != 0 ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::CUR_COL) . ' = ' . $new_seq['start'];
    }

    if ( $new_seq['cycle'] == null && $old_seq['cycle'] != null ) {
      $sql[] = h2::get_quoted_column_name(h2_sequence::CYC_COL) . ' = DEFAULT';
    }
    elseif ( $new_seq['cycle'] != null && strcasecmp($new_seq['cycle'], $old_seq['cycle']) != 0 ) {
      $value = strcasecmp($new_seq['cycle'], 'false') == 0 ? 'FALSE' : 'TRUE';
      $sql[] = h2::get_quoted_column_name(h2_sequence::CYC_COL) . ' = ' . $value;
    }

    if ( ! empty($sql) ) {
      $out = "UPDATE " . h2::get_quoted_table_name(h2_sequence::TABLE_NAME);
      $out .= "\nSET " . implode(",\n    ", $sql);
      $out .= "\nWHERE " . h2::get_quoted_column_name(h2_sequence::SEQ_COL) . " = ";
      $out .= "'" . $old_seq['name'] . "'";
      $ofs->write("$out;\n");
    }
  }
