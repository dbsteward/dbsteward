...
class h2_function extends sql99_function {
  ...
  public static function get_creation_sql($node_schema, $node_function) {
    ...
    if ( h2::$swap_function_delimiters ) {
    ...
    if ( isset($node_function->functionParameter) ) {
      ...
      foreach ( $node_function->functionParameter as $param ) {
        ...
        $type = $param['type'];
        if ( $node_type = h2_type::get_type_node(dbsteward::$new_database, $node_schema, $type) ) {
          $type = h2_type::get_enum_type_declaration($node_type);
        }
        ...
        $sparam .= h2::get_quoted_function_parameter($param['name']) . ' ' . $type;
        $params[] = $sparam;
      }
      $sql .= implode(', ', $params);
    }
    ...
    if (!static::is_procedure($node_function)) {
      ...
      if ( $node_type = h2_type::get_type_node(dbsteward::$new_database, $node_schema, $returns) ) {
        $returns = h2_type::get_enum_type_declaration($node_type);
      }
      $sql .= "RETURNS " . $returns . "\n";
    }
    $sql .= "LANGUAGE SQL\n";
    ...
    // unlike pgsql8, h2 defaults to SECURITY DEFINER, so we need to set it to INVOKER unless explicitly told to leave it DEFINER
    ...
    if ( !empty($node_function['description']) ) {
      $sql .= "COMMENT " . h2::quote_string_value($node_function['description']) . "\n";
    }
  ...
  public static function get_drop_sql($node_schema, $node_function) {
    if ( ! static::has_definition($node_function) ) {
      $note = "Not dropping function '{$node_function['name']}' - no definitions for h2";
      dbsteward::warning($note);
      return "-- $note\n";
    }
    $function_type = static::is_procedure($node_function) ? 'PROCEDURE' : 'FUNCTION';
    return "DROP $function_type IF EXISTS " . h2::get_fully_qualified_object_name($node_schema['name'], $node_function['name'], 'function') . ";";
  }

  public static function get_declaration($node_schema, $node_function) {
    return h2::get_fully_qualified_object_name($node_schema['name'], $node_function['name'], 'function');
  }
