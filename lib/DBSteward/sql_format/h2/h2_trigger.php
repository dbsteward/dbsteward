...
class h2_trigger extends sql99_trigger {
  public static function get_creation_sql($node_schema, $node_trigger) {
    ...
    foreach ( $events as $event ) {
      ...
      $trigger_name = h2::get_fully_qualified_table_name($node_schema['name'], $node_trigger['name'] . ($single? '' : "_$event"));
...
SQL;
  ...
  public static function get_drop_sql($node_schema, $node_trigger) {
    ...
    $events = self::get_events($node_trigger);
    if ( count($events) == 1 ) {
      return "DROP TRIGGER IF EXISTS " . h2::get_fully_qualified_table_name($node_schema['name'], $node_trigger['name']) . ";\n";
    }
    else {
      $ddl = "";
      foreach ( $events as $event ) {
        if ( $event = self::validate_event($event) ) {
          $ddl .= "DROP TRIGGER IF EXISTS " . h2::get_fully_qualified_table_name($node_schema['name'], $node_trigger['name'] . "_$event") . ";\n";
        }
      }
      return $ddl;
    }
