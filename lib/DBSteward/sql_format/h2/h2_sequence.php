...
class h2_sequence {
  ...
  public static function get_creation_sql($node_schema, $node_sequences) {
    ...
    $table_name = h2::get_quoted_table_name(self::TABLE_NAME);
    $seq_col = h2::get_quoted_column_name(self::SEQ_COL);
    $inc_col = h2::get_quoted_column_name(self::INC_COL);
    $min_col = h2::get_quoted_column_name(self::MIN_COL);
    $max_col = h2::get_quoted_column_name(self::MAX_COL);
    $cur_col = h2::get_quoted_column_name(self::CUR_COL);
    $start_col = h2::get_quoted_column_name(self::START_COL);
    $cyc_col = h2::get_quoted_column_name(self::CYC_COL);
...
SQL;
  ...
  public static function get_shim_creation_sql() {
    ...
    $cyc_col = h2::get_quoted_column_name(self::CYC_COL);
    $start_col = h2::get_quoted_column_name(self::START_COL);
    $adv_col = h2::get_quoted_column_name(self::ADV_COL);

    $delimiter_begin = h2::$swap_function_delimiters ? ("DELIMITER ".h2_function::ALT_DELIMITER."\n") : '';
    $delimiter_end = h2::$swap_function_delimiters ? "\nDELIMITER ;" : '';
    $delimiter = h2::$swap_function_delimiters ? h2_function::ALT_DELIMITER : ';';
