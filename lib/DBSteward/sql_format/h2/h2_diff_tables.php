...
class h2_diff_tables extends sql99_diff_tables {
  /**
  ...
  public static function diff_data($ofs, $old_schema, $new_schema) {
    foreach(dbx::get_tables($new_schema) AS $new_table) {
      ...
      if ( h2_diff_tables::is_renamed_table($new_schema, $new_table) ) {
        $renamed_table_old_schema = h2_table::get_old_table_schema($new_schema, $new_table);
        $renamed_table_old_table = h2_table::get_old_table($new_schema, $new_table);
        $ofs->write(
        ...
        if ( $old_schema != null && h2_schema::contains_table($old_schema, $new_table['name']) ) {
          $old_table = dbx::get_table($old_schema, $new_table['name']);
        }
        $ofs->write(
  ...
  private static function create_tables($ofs, $old_schema, $new_schema, $old_table = null, $new_table = null) {
    foreach(dbx::get_tables($new_schema) as $table) {
      ...
      if (($old_schema == null) || !h2_schema::contains_table($old_schema, $table['name'])) {
        if ( !dbsteward::$ignore_oldnames && h2_diff_tables::is_renamed_table($new_schema, $table) ) {
          // oldTableName renamed table ? rename table instead of create new one
          $old_table_name = h2::get_fully_qualified_table_name($table['oldSchemaName'] ?: $old_schema['name'], $table['oldTableName'] ?: $table['name']);
          $new_table_name = h2::get_fully_qualified_table_name($new_schema['name'], $table['name']);
          $ofs->write("-- table rename from oldTableName specification\n");
          $ofs->write("ALTER TABLE $old_table_name RENAME TO $new_table_name;\n");
        }
        else {
          $ofs->write(h2_table::get_creation_sql($new_schema, $table) . "\n");
        }
      }
    }
  }
  ...
  protected static function is_moved_table($schema, $table) {
    if (!isset($table['oldSchemaName']) || !h2::$use_schema_name_prefix) {
  ...
  private static function update_table_partitions($ofs1, $ofs3, $old_schema, $old_table, $new_schema, $new_table) {
    $new_part = h2_table::get_partition_sql($new_schema, $new_table);
    $old_part = h2_table::get_partition_sql($old_schema, $old_table);

    $table_name =  h2::get_fully_qualified_table_name($new_schema, $new_table['name']);
  ...
  private static function update_table_columns($ofs1, $ofs3, $old_schema, $old_table, $new_schema, $new_table) {
    ...
    //
    // For these reasons, the h2 implementation of this function is optimized for making as few
    ...
    foreach(dbx::get_table_columns($old_table) as $old_column) {
      if (!h2_table::contains_column($new_table, $old_column['name'])) {
        if ( !dbsteward::$ignore_oldnames && ($renamed_column_name = h2_table::column_name_by_old_name($new_table, $old_column['name'])) !== false ) {
          continue;
        }
        else {
    ...
    foreach ($new_columns as $col_index => $new_column) {
      $cmd1 = array(
        'command' => 'nothing',
        'column' => $new_column,
        'defaults' => h2_diff::$add_defaults,
        'nulls' => TRUE,
        'auto_increment' => FALSE
      );

      if (!h2_table::contains_column($old_table, $new_column['name'], TRUE)) {
        // column not present in old table, is either renamed or new
        if (!dbsteward::$ignore_oldnames && h2_diff_tables::is_renamed_column($old_table, $new_table, $new_column)) {
          // renamed
          $cmd1['command'] = 'change';
        ...
        $old_column_type = h2_column::column_type(dbsteward::$old_database, $old_schema, $old_table, $old_column);
        $new_column_type = h2_column::column_type(dbsteward::$new_database, $new_schema, $new_table, $new_column);
        ...
        $auto_increment_added = !h2_column::is_auto_increment($old_column['type']) && h2_column::is_auto_increment($new_column['type']);
        $auto_increment_removed = h2_column::is_auto_increment($old_column['type']) && !h2_column::is_auto_increment($new_column['type']);
        ...
        elseif ($default_changed) {
          if (strlen($new_default) > 0) {
            if (h2_column::is_timestamp($new_column)) {
    ...
    $table_name = h2::get_fully_qualified_table_name($new_schema['name'], $new_table['name']);

    $get_command_sql = function ($command) use (&$new_schema, &$new_table) {
      if ($command['command'] == 'nothing') return NULL;

      if ($command['command'] == 'drop') {
        $name = h2::get_quoted_column_name($command['column']['name']);
        return "DROP COLUMN $name";
      }

      $defn = h2_column::get_full_definition(
      ...
      if ($command['command'] == 'change') {
        $old = h2::get_quoted_column_name($command['old']);
      ...
      if ($command['command'] == 'create') {
        ...
        elseif (array_key_exists('after', $command)) {
          $col = h2::get_quoted_column_name($command['after']);
    ...
    foreach ($defaults['set'] as $column) {
      $name = h2::get_quoted_column_name($column['name']);

      if (strlen($column['default']) > 0) {
        $default = (string)$column['default'];
      }
      else {
        $type = h2_column::column_type(dbsteward::$new_database, $new_schema, $new_table, $column);
        $default = h2_column::get_default_value($type);
      }
  ...
  public static function drop_tables($ofs, $old_schema, $new_schema, $old_table = null, $new_table = null) {
    if ($old_schema != null ) {
      ...
      foreach($tables as $table) {
        ...
        if (!h2_schema::contains_table($new_schema, $table['name'])) {
          // if the table was renamed, don't drop it
          if ( !dbsteward::$ignore_oldnames
            && h2_schema::table_formerly_known_as(dbsteward::$new_database, $old_schema, $table, $reformed_schema, $reformed_table) ) {
            $old_table_name = h2::get_fully_qualified_table_name($old_schema['name'], $table['name']);
            $reformed_table_name = h2::get_fully_qualified_table_name($reformed_schema['name'], $reformed_table['name']);
            $ofs->write("-- DROP TABLE $old_table_name omitted: new table $reformed_table_name indicates it is her replacement\n");
          }
          else {
            $ofs->write(h2_table::get_drop_sql($old_schema, $table) . "\n");
          }
        }
      }
    }
  ...
  public static function get_data_sql($old_schema, $old_table, $new_schema, $new_table, $delete_mode = false) {
    ...
    if ( $old_table == null ) {
      if ( !$delete_mode ) {
        ...
        $sql .= h2_column::get_serial_start_dml($new_schema, $new_table);
      }
    }
    else {
      ...
      if ( $delete_mode ) {
        ...
        if ( $old_table_rows && $new_table_rows ) {
          $new_table_primary_keys = h2_table::primary_key_columns($new_table);
  ...
  private static function get_data_row_insert($node_schema, $node_table, $data_row_columns, $data_row) {
    ...
    for($i=0; $i < $data_row_columns_count; $i++) {
      $columns[] = h2::get_quoted_column_name($data_row_columns[$i]);
      $values[] = h2::column_value_default($node_schema, $node_table, $data_row_columns[$i], $data_row->col[$i]);
    }
  ...
  public static function table_data_rows_compare($table_a, $table_b, $in_both, &$a_rows, &$b_rows, &$changes) {
    ...
    if ( $table_a_data_rows == null || $table_a_data_rows_columns == null ) {
      ...
      $table_b_primary_keys = h2_table::primary_key_columns($table_b);
...
// echo "rows don't match\n";  var_dump($table_a_data_row);  var_dump($table_b_data_row);
  ...
  public static function apply_table_options_diff($ofs1, $ofs3, $schema, $table, $alter_options, $create_options, $drop_options) {
    $fq_name = h2::get_fully_qualified_table_name($schema['name'], $table['name']);
    ...
    if (!empty($drop_options)) {
      ...
      $sql.= h2_diff_tables::get_recreate_table_sql($schema, $table);
      $ofs1->write($sql."\n");
    }
    else {
      ...
      if (!empty($alter_create)) {
        ...
        $sql.= h2_table::get_table_options_sql($alter_create).";";
        $ofs1->write($sql."\n");
      }
    }
  }
  ...
  private static function get_recreate_table_sql($schema, $table) {
    $fq_name = h2::get_fully_qualified_table_name($schema['name'], $table['name']);
    $fq_tmp_name = h2::get_fully_qualified_table_name($schema['name'], $table['name'].'_DBSTEWARD_MIGRATION');
    ...
    $opt_sql = h2_table::get_table_options_sql(h2_table::get_table_options($schema, $table));
    if (!empty($opt_sql)) {
      $sql .= "\n" . $opt_sql;
    }

    if ( strlen($table['description']) > 0 ) {
      $sql .= "\nCOMMENT " . h2::quote_string_value($table['description']);
    }
