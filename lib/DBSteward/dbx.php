    ...
    if ($type == 'all' || $type == 'primaryKey') {
      if (isset($node_table['primaryKey'])) {
        ...
        foreach ($primary_key_columns AS $primary_key_column) {
          $primary_key_list .= pgsql8::get_quoted_name($primary_key_column, dbsteward::$quote_column_names) . ', ';
        }
  ...
  public static function primary_key_expression($node_schema, $node_table, $data_row_columns, $data_row) {
    ...
    for ($i = 0; $i < count($primary_keys); $i++) {
      ...
      if (strcasecmp(dbsteward::get_sql_format(), 'pgsql8') == 0) {
        ...
        $primary_key_expression .= pgsql8::get_quoted_name($primary_column_name, dbsteward::$quote_column_names) . ' = ' . pgsql8::value_escape($value_type, $data_row->col[$column_index]);
      }
